...
def restart_remote_runtime(
    *,
    ...
    from ss_ssh_e2e.diagnostics import ps, ssh_capture
...
def _redeem_task_code(
    *,
    ...
    return resp.status_code, _http_json(resp)
...
def _redeem_headers_for_recoverability(
    *,
    ...
    _record_json_evidence(
        out_dir=out_dir,
        filename="recoverability.redeem.json",
...
def _verify_job_terminal_status(
    *,
    ...
    job_status, job_payload = _get_job(
    ...
    if str(job_payload.get("status", "")) != expected_terminal_status:
...
def recoverability_check(
    *,
    ...
    with ssh_tunnel(
        ...
        remote_host=DEFAULT_REMOTE_API_HOST,
        remote_port=DEFAULT_REMOTE_API_PORT,
