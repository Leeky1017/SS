...
- [ ] Define the tenant model and required request context (how tenant identity is determined)
- [ ] Persistence is tenant-isolated and prevents collisions (including same `job_id` across tenants)
- [ ] Document the migration/compatibility plan for existing single-tenant deployments
- [ ] Tests cover at least one isolation invariant (no cross-tenant access)
- [ ] Implementation run log records `ruff check .`, `pytest -q`, and deployment notes
