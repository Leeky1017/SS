...
class ScriptedStataRunner(StataRunner):
    def __init__(
        self,
        *,
        jobs_dir: Path,
        scripted_ok: Sequence[bool] | None = None,
        scripted_executions: Sequence[Execution] | None = None,
    ) -> None:
        self._jobs_dir = Path(jobs_dir)
        self._scripted_ok = list(scripted_ok) if scripted_ok is not None else None
        self._scripted_executions = (
            list(scripted_executions) if scripted_executions is not None else None
        )
        self._ok_cursor = 0
        self._exec_cursor = 0
    ...
    def _next_execution(self) -> Execution:
        scripted = self._scripted_executions
        if scripted is not None:
            if len(scripted) == 0:
                return self._execution_for(ok=True)
            if self._exec_cursor >= len(scripted):
                return scripted[-1]
            execution = scripted[self._exec_cursor]
            self._exec_cursor += 1
            return execution
        return self._execution_for(ok=self._next_ok())

    def _next_ok(self) -> bool:
        if self._scripted_ok is None:
            return True
        if self._ok_cursor >= len(self._scripted_ok):
            return bool(self._scripted_ok[-1])
        ok = bool(self._scripted_ok[self._ok_cursor])
        self._ok_cursor += 1
        return ok
