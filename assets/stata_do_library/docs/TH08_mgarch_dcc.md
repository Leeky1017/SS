# TH08_mgarch_dcc — DCC多元GARCH

## 任务元信息

| 属性 | 值 |
|------|-----|
| **模板ID** | TH08 |
| **Slug** | mgarch_dcc |
| **名称(中文)** | DCC多元GARCH |
| **Name(EN)** | DCC-GARCH Model |
| **家族** | time_series |
| **等级** | L2 |
| **版本** | 2.0.0 |

## 功能描述

DCC multivariate GARCH model

## 使用场景

- 关键词：dcc, mgarch, multivariate, volatility

## 输入

| 文件 | 角色 | 必需 |
|------|------|------|
| data.csv | main_dataset | 是 |

## 参数与占位符

| 占位符 | 类型 | 必需 | 说明 |
|--------|------|------|------|
| `__VARS__` | string | 是 | Variables |
| `__TIMEVAR__` | string | 是 | Time variable |

## 输出

| 文件 | 类型 | 说明 |
|------|------|------|
| table_TH08_dcc.csv | table | DCC results |
| data_TH08_dcc.dta | data | Output data |
| result.log | log | Execution log |

## 依赖

（无额外依赖）

## 示例

```stata
* Template: TH08_mgarch_dcc
* Script: assets/stata_do_library/do/TH08_mgarch_dcc.do
* 将占位符替换为你的变量名/参数，然后交由执行器运行。
```

