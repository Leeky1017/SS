...
* - 2026-01-09 (Issue #263): Add input validation (>=2 vars, lags/steps>=1) and `tsset`/gap diagnostics.
*   增加输入校验（>=2 变量，lags/steps>=1）与 tsset/缺口诊断。
* - 2026-01-09 (Issue #263): Keep fallback plot when IRF graph fails, but emit explicit warnings (no silent failure).
*   IRF 图失败时保留兜底图，但必须显式输出 warn（禁止静默失败）。
* ==============================================================================
...
display "SS_BP_REVIEW|issue=263|template_id=TH11|ssc=none|output=png_dta|policy=warn_fail"
display "SS_DEP_CHECK|pkg=stata|source=built-in|status=ok"

program define ss_fail_TH11
    args code cmd msg
    timer off 1
    ...
    display "SS_TASK_END|id=TH11|status=fail|elapsed_sec=`elapsed'"
    capture log close
    local rc_log = _rc
    if `rc_log' != 0 {
...
if _rc {
    ss_fail_TH11 601 "confirm file data.csv" "input_file_not_found"
}
...
if _rc {
    ss_fail_TH11 111 "confirm variable `timevar'" "time_var_missing"
}
foreach v of local vars {
    capture confirm variable `v'
    if _rc {
        ss_fail_TH11 111 "confirm variable `v'" "series_var_missing"
    }
}
local n_vars : word count `vars'
if `n_vars' < 2 {
    ss_fail_TH11 198 "validate vars count" "need_at_least_two_series"
}
if missing(`lags') | `lags' < 1 {
    ss_fail_TH11 198 "validate lags" "lags_invalid"
}
if missing(`steps') | `steps' < 1 {
    ss_fail_TH11 198 "validate steps" "steps_invalid"
}
...
capture tsset `tsvar'
if _rc {
    ss_fail_TH11 `=_rc' "tsset `tsvar'" "tsset_failed"
}
...
if _rc {
    ss_fail_TH11 `=_rc' "var `vars', lags(1/`lags')" "var_failed"
}
else {
...
local rc_gexp = _rc
if `rc_gexp' != 0 {
    display "SS_RC|code=`rc_gexp'|cmd=graph export fig_TH11_irf.png|msg=graph_export_failed|severity=warn"
    ...
    local rc_gexp2 = _rc
    if `rc_gexp2' != 0 {
        display "SS_RC|code=`rc_gexp2'|cmd=graph export fig_TH11_irf.png|msg=graph_export_failed|severity=warn"
    }
}
