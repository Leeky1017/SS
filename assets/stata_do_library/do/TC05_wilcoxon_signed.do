...
if _rc {
    ...
    local elapsed = r(t1)
    display "SS_TASK_END|id=TC05|status=fail|elapsed_sec=`elapsed'"
    log close
    exit `rc'
}
...
capture noisily signrank `var1' = `var2'
if _rc {
    local rc = _rc
    display "SS_RC|code=`rc'|cmd=signrank|msg=test_failed|severity=fail"
    timer off 1
    quietly timer list 1
    local elapsed = r(t1)
    display "SS_TASK_END|id=TC05|status=fail|elapsed_sec=`elapsed'"
