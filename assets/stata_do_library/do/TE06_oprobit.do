...
* - [x] Ordered outcome assumption noted (有序因变量假设；阈值解释)
...
if _rc {
    ...
    local elapsed = r(t1)
    display "SS_TASK_END|id=TE06|status=fail|elapsed_sec=`elapsed'"
    log close
    exit `rc'
}
...
capture noisily oprobit `depvar' `indepvars'
if _rc {
    local rc = _rc
    display "SS_RC|code=`rc'|cmd=oprobit|msg=fit_failed|severity=fail"
    timer off 1
    quietly timer list 1
    local elapsed = r(t1)
    display "SS_TASK_END|id=TE06|status=fail|elapsed_sec=`elapsed'"
