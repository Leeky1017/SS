...
* DEPENDENCIES:
*   - stata source=built-in purpose="balance diagnostics (weighted/unweighted) + overlap plots"
* ==============================================================================
...
* - Best practice: use standardized differences (and optional weighted stats) to assess balance; complement with PS overlap plots. /
*   最佳实践：用标准化差异（可选加权）评估平衡性，并辅以倾向得分重叠图。
* - SSC deps: none (this template does not require matching packages) / SSC 依赖：无（本模板不依赖匹配包）
* - Error policy: fail on missing treatment/covariates; warn if many covariates fail balance threshold /
*   错误策略：缺少处理变量/协变量→fail；大量变量未通过阈值→warn
* ==============================================================================
display "SS_BP_REVIEW|issue=247|template_id=TG07|ssc=none|output=csv_png|policy=warn_fail"
