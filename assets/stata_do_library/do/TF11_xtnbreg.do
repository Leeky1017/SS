...
program define ss_fail_TF11
    ...
    display "SS_TASK_END|id=TF11|status=fail|elapsed_sec=`elapsed'"
    capture log close
    if _rc != 0 {
...
* - Best practice: use FE when feasible; fall back to RE if FE fails (common for NB with many groups). /
*   最佳实践：可行时优先 FE；若 FE 失败（面板 NB 常见），回退到 RE 并记录告警。
...
* ==============================================================================
display "SS_BP_REVIEW|issue=246|template_id=TF11|ssc=none|output=csv|policy=warn_fail"
...
if _rc {
    ss_fail_TF11 601 "confirm file data.csv" "input_file_not_found"
}
...
if _rc {
    ss_fail_TF11 111 "confirm variable `panelvar'" "panel_var_missing"
}
capture confirm variable `timevar'
if _rc {
    ss_fail_TF11 111 "confirm variable `timevar'" "time_var_missing"
}
capture xtset `panelvar' `timevar'
if _rc {
    ss_fail_TF11 `=_rc' "xtset `panelvar' `timevar'" "xtset_failed"
}
tempvar _ss_n_i
...
capture noisily xtnbreg `depvar' `indepvars', fe
if _rc {
    display "SS_RC|code=`=_rc'|cmd=xtnbreg fe|msg=xtnbreg_fe_failed_fallback_re|severity=warn"
    capture noisily xtnbreg `depvar' `indepvars', re
    if _rc {
        ss_fail_TF11 `=_rc' "xtnbreg" "xtnbreg_failed"
    }
}
local ll = e(ll)
...
gen double ll = `ll'
capture export delimited using "table_TF11_xtnb.csv", replace
if _rc {
    ss_fail_TF11 `=_rc' "export delimited" "export_failed"
}
...
capture save "data_TF11_xtnb.dta", replace
if _rc {
    ss_fail_TF11 `=_rc' "save" "save_failed"
}
