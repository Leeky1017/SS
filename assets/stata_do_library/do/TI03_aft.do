* ==============================================================================
* SS_TEMPLATE: id=TI03  level=L1  module=I  title="AFT Model"
* INPUTS:
*   - data.csv  role=main_dataset  required=yes
* OUTPUTS:
*   - table_TI03_aft.csv type=table desc="AFT results"
*   - data_TI03_aft.dta type=data desc="Output data"
...
display "SS_TASK_BEGIN|id=TI03|level=L1|title=AFT_Model"
...
local rc_file = _rc
if `rc_file' != 0 {
    ss_fail TI03 601 "confirm file data.csv" "input_file_not_found"
}
...
local rc_stset = _rc
if `rc_stset' != 0 {
    ss_fail TI03 `rc_stset' "stset" "stset_failed"
}
quietly count if _d == 1
local n_events = r(N)
display "SS_METRIC|name=n_events|value=`n_events'"
if `n_events' == 0 {
    ss_fail TI03 200 "stset" "no_failure_events"
}
if `n_events' < 5 {
...
capture noisily streg `indepvars', dist(lognormal) time
local rc_streg = _rc
if `rc_streg' != 0 {
    if `rc_streg' == 430 {
        ...
        ss_fail TI03 `rc_streg' "streg" "streg_failed"
    }
}
local ll = .
...
clear
set obs 1
gen str32 model = "AFT Lognormal"
gen double ll = `ll'
export delimited using "table_TI03_aft.csv", replace
display "SS_OUTPUT_FILE|file=table_TI03_aft.csv|type=table|desc=aft_results"
restore

local n_output = _N
display "SS_METRIC|name=n_output|value=`n_output'"
save "data_TI03_aft.dta", replace
display "SS_OUTPUT_FILE|file=data_TI03_aft.dta|type=data|desc=aft_data"
...
display "SS_TASK_END|id=TI03|status=ok|elapsed_sec=`elapsed'"
log close
