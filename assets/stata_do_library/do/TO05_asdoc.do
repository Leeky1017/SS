* ==============================================================================
* SS_TEMPLATE: id=TO05  level=L2  module=O  title="Asdoc"
* INPUTS:
...
capture which asdoc
if _rc {
    display "SS_DEP_MISSING:asdoc"
    display "SS_ERROR:DEP_MISSING:asdoc not installed"
    display "SS_ERR:DEP_MISSING:asdoc not installed"
    log close
    exit 199
}
display "SS_DEP_CHECK|pkg=asdoc|source=ssc|status=ok"
...
asdoc regress `depvar' `indepvars', save(table_TO05_asdoc.doc) replace ///
    title(Regression Results) nest stat(N r2 r2_a)
