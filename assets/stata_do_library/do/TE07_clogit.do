...
* - [x] Conditional logit needs proper grouping (条件 logit 需正确的 group 变量)
...
if _rc {
    ...
    local elapsed = r(t1)
    display "SS_TASK_END|id=TE07|status=fail|elapsed_sec=`elapsed'"
    log close
    exit `rc'
}
...
capture noisily clogit `depvar' `indepvars', group(`group_var')
if _rc {
    local rc = _rc
    display "SS_RC|code=`rc'|cmd=clogit|msg=fit_failed|severity=fail"
    timer off 1
    quietly timer list 1
    local elapsed = r(t1)
    display "SS_TASK_END|id=TE07|status=fail|elapsed_sec=`elapsed'"
