...
* - Date: 2026-01-10
* - Model assumptions / 模型假设: parametric exponential; compare fit vs other dists if needed / 指数模型假设常数风险率，必要时对比其他分布
...
local rc_file = _rc
if `rc_file' != 0 {
    ss_fail TI02 601 "confirm file data.csv" "input_file_not_found"
}
...
local rc_time = _rc
if `rc_time' != 0 {
    ss_fail TI02 200 "confirm numeric variable `timevar'" "timevar_missing_or_not_numeric"
}
capture confirm numeric variable `failvar'
local rc_fail = _rc
if `rc_fail' != 0 {
    ss_fail TI02 200 "confirm numeric variable `failvar'" "failvar_missing_or_not_numeric"
}
...
local rc_stset = _rc
if `rc_stset' != 0 {
    ss_fail TI02 `rc_stset' "stset" "stset_failed"
}
quietly count if _d == 1
local n_events = r(N)
display "SS_METRIC|name=n_events|value=`n_events'"
if `n_events' == 0 {
    ss_fail TI02 200 "stset" "no_failure_events"
}
if `n_events' < 5 {
...
capture noisily streg `indepvars', dist(`dist')
local rc_streg = _rc
if `rc_streg' != 0 {
    if `rc_streg' == 430 {
        ...
        ss_fail TI02 `rc_streg' "streg" "streg_failed"
    }
}
local ll = .
