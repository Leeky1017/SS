...
* - 2026-01-08: Keep explicit train/test split by time and report forecast error metrics (保留按时间划分训练/测试并报告预测误差).
* ==============================================================================
...
program define ss_fail_T39
    ...
    display "SS_TASK_END|id=T39|status=fail|elapsed_sec=`elapsed'"
    capture log close
    if _rc != 0 {
...
if _rc {
    ...
    if _rc {
        ...
        ss_fail_T39 601 "confirm file" "data_file_not_found"
    }
