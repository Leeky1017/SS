...
if _rc {
    ...
    local elapsed = r(t1)
    display "SS_TASK_END|id=TC06|status=fail|elapsed_sec=`elapsed'"
    log close
    exit `rc'
}
...
capture noisily tabulate `var1' `var2', exact
if _rc {
    local rc = _rc
    display "SS_RC|code=`rc'|cmd=tabulate exact|msg=test_failed|severity=fail"
    timer off 1
    quietly timer list 1
    local elapsed = r(t1)
    display "SS_TASK_END|id=TC06|status=fail|elapsed_sec=`elapsed'"
