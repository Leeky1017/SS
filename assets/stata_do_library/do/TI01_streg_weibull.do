...
* - Date: 2026-01-10
* - Model assumptions / 模型假设: parametric Weibull; check fit via IC (AIC) / 参数模型，建议用信息准则对比拟合
* - PH assumption / 比例风险: N/A (not Cox) / 不适用（非 Cox）
* - Competing risks / 竞争风险: N/A / 不适用
* - SSC deps / SSC 依赖: none / 无
* - Guardrails / 防御: stset fail-fast + small-events warning + convergence warning
* ------------------------------------------------------------------------------
...
local rc_time = _rc
if `rc_time' != 0 {
    ss_fail TI01 200 "confirm numeric variable `timevar'" "timevar_missing_or_not_numeric"
}
capture confirm numeric variable `failvar'
local rc_fail = _rc
if `rc_fail' != 0 {
    ss_fail TI01 200 "confirm numeric variable `failvar'" "failvar_missing_or_not_numeric"
}
