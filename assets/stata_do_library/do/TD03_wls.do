...
* - [x] Robust diagnostics on failure (拟合失败输出 `SS_RC|...`)
...
if _rc {
    ...
    local elapsed = r(t1)
    display "SS_TASK_END|id=TD03|status=fail|elapsed_sec=`elapsed'"
    log close
    exit `rc'
}
...
capture noisily regress `depvar' `indepvars' [aweight=`weight_var']
if _rc {
    local rc = _rc
