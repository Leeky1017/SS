...
* - 2026-01-08: Keep ADF/PP unit-root tests and document lag choices (保留单位根检验并明确滞后阶数选择).
* ==============================================================================
...
program define ss_fail_T37
    ...
    display "SS_TASK_END|id=T37|status=fail|elapsed_sec=`elapsed'"
    capture log close
    if _rc != 0 {
...
if _rc {
    ...
    if _rc {
        ...
        ss_fail_T37 601 "confirm file" "data_file_not_found"
    }
