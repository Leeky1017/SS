...
* - Date: 2026-01-10
* - Design / 设计: SMB depends on size measure and rebalancing frequency
* - Data checks / 数据校验: missing size/returns; extreme size can dominate sorts
...
if _rc {
    ss_fail_TK17 200 confirm_numeric return_not_numeric `return_var' S02_validate_inputs
}
capture confirm numeric variable `size_var'
if _rc {
    ss_fail_TK17 200 confirm_numeric size_not_numeric `size_var' S02_validate_inputs
}
foreach var in `return_var' `size_var' {
