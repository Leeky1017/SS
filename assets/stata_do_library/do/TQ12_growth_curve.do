...
if _rc {
    ...
    display "SS_TASK_END|id=TQ12|status=fail|elapsed_sec=."
    log close
    exit 601
}
...
capture mixed `depvar' `time_var' `indepvars' || `id_var': `time_var', covariance(independent)
if _rc {
    local rc_mixed = _rc
    display "SS_RC|code=`rc_mixed'|cmd=mixed|msg=mixed_fit_failed_fallback|severity=warn"
    capture mixed `depvar' `time_var' `indepvars' || `id_var':
    if _rc {
        local rc_mixed2 = _rc
        display "SS_RC|code=`rc_mixed2'|cmd=mixed|msg=mixed_fit_failed|severity=fail"
        display "SS_TASK_END|id=TQ12|status=fail|elapsed_sec=."
        log close
        exit `rc_mixed2'
    }
...
predict yhat, fitted
twoway (scatter `depvar' `time_var', mcolor(gray%30)) ///
       (line yhat `time_var', sort lcolor(red)), ///
    title("Growth Curve Model") legend(order(1 "Observed" 2 "Fitted"))
graph export "fig_TQ12_growth.png", replace width(1200)
...
save "data_TQ12_growth.dta", replace
