...
capture which twopm
if _rc {
    display "SS_DEP_CHECK|pkg=twopm|source=ssc|status=missing"
    display "SS_DEP_MISSING|pkg=twopm"
    display "SS_RC|code=199|cmd=which twopm|msg=dependency_missing|severity=fail"
    timer off 1
...
capture noisily twopm `depvar' `indepvars', firstpart(logit) secondpart(glm, family(gamma) link(log))
if _rc {
    local rc = _rc
    display "SS_RC|code=`rc'|cmd=twopm|msg=fit_failed|severity=fail"
    timer off 1
    quietly timer list 1
    local elapsed = r(t1)
    display "SS_TASK_END|id=TE05|status=fail|elapsed_sec=`elapsed'"
